...
const a = @SMatrix [[√3/2, 1/2] [-√3/2, 1/2] [0.0, -1.0]]
const b = [a[:,2].-a[:,3]  a[:,3].-a[:,1] a[:,1].-a[:,2]]

# Pauli Matrix
const s0 = SMatrix{2,2}(1.0, 0.0im, 0.0, 1.0)
const sx = SMatrix{2,2}(0.0im, 1, 1, 0.0)
const sy = SMatrix{2,2}(0.0, 1.0im, -1.0im, 0.0)
const sz = SMatrix{2,2}(1.0, 0.0im, 0.0, -1.0)

# ----------------------------------
# Ref: Shen Shunqing, Topological insulators, Dirac equation in condensed matter, 2ed, chapter 6
struct Para{T<:Real}
    t1::T
    t2::T
    ϕ ::T
